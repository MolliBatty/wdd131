...
function getStars(rating) {
    let stars = '';
    for (let i = 1; i <= 5; i++) {
        stars += i <= rating ? '<span aria-hidden="true">⭐</span>' : '<span aria-hidden="true">☆</span>';
    }
    return stars;
}
...
function showRandomRecipe() {
    ...
    div.innerHTML = buildRecipeCard(randomRecipe);
    document.querySelector('main').appendChild(div.firstElementChild);
}

function search(){
    const recipeQuery = document.querySelector('#search').value.trim();
    ...
    document.querySelectorAll('.recipe, .no-results').forEach(el => el.remove());
    ...
    if(filteredRecipes.length === 0){
        ...
        filteredRecipes.forEach(recipe => {
            const div = document.createElement('div');
            div.innerHTML = buildRecipeCard(recipe);
            document.querySelector('main').appendChild(div.firstElementChild);
        });
    }
}
